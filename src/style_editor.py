...
- 말투를 입히더라도 문장의 정보량을 줄이지 않는다. 원문의 구체 디테일은 유지한다.
- 같은 감탄사, 같은 어미, 같은 형용사를 연속해서 반복하지 않는다.
- `좋았다`, `인상적이었다`, `분위기가 좋았다`처럼 밋밋한 감상은 원문 단서 안에서 조금 더 구체적으로 다듬는다.
- `사진 속 장면`, `하루의 분위기`, `흐름을 이어준다` 같은 템플릿성 문구가 있으면 더 자연스러운 표현으로 고친다.
...
- 마지막으로 스스로 한 번 퇴고해 어색한 반복과 빈말을 줄인다.
